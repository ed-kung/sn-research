...
\noindent \emph{Remark.} Since $\hat{\theta}$ is the average of the zap surprises across previous posts, Proposition \ref{prop_v4v} suggests that we can test for the presence of learning by regressing the quality of a user's next post on the average of zap surprises for all prior posts made by that user. Doing so would require a measure of post quality $q_i$ which is not equivalent to the zap amount received by that post. Equation \eqref{eq_zi} suggests a parsimonious model for estimating $q_i$ for each post:
\begin{align}
