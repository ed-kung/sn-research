...
By Bayes' Rule, $f(q, \epsilon) = f_{\epsilon|q} (\epsilon | q) f_q (q)$, where $f_{\epsilon|q}(\cdot)$ is the conditional density of $\epsilon$ given $q$, and $f_q(\cdot)$ is the marginal density of $q$. We assume that $f_{\epsilon|q}(\cdot)$ weakly satisfies the monotone likelihood ratio property in $\epsilon$ with respect to $q$:
...
Supposing the user has a discount factor of $\rho$, the expected present value of the Poisson stream of zaps is equal to:\footnote{Please refer to the appendix for a derivation.}
\begin{align}
...
\begin{proposition}
Suppose assumptions 1 and 2 hold. Then an increase in $c$:
...
\begin{proof}
A post idea $(q,\epsilon)$ is posted if and only if $\epsilon > c - V(q)$. Let $S(\epsilon|q) = \text{Pr}\left(\epsilon^\prime > \epsilon | q \right)$ denote the conditional survival function of $\epsilon$ given $q$, and let $h(q)$ denote the marginal density of $q$.

The probability of posting conditional on receiving an idea is:
\begin{align*}
P(c) = \int S(c-V(q) | q) h(q) dq
\end{align*}

\begin{enumerate}[label=(\roman*)]
\item Increasing $c$ raises $c - V(q)$ for every $q$. Since $S(\cdot | q)$ is strictly decreasing, $S(c - V(q) | q)$ strictly falls for all $q$, so $P(c)$ strictly falls. Expected posts per unit of time are $\gamma P(c)$, which strictly decreases.

\item Expected quality conditional on posting is:
\begin{align*}
E[q| \text{post},c] = \frac{\int q S(c - V(q) | q) h(q) dq}{\int S(c - V(q) | q) h(q)dq}
\end{align*}
Taking the derivative with respect to $c$, the sign of $\frac{d}{dc} E[q|\text{post},c]$ is equal to the sign of:
\begin{align*}
& \left[ \int q f(c - V(q) | q)h(q) dq \right] \left[\int S(c - V(q) | q) h(q) dq \right] \\
& - \left[ \int q S(c - V(q) | q) h(q) dq \right]\left[\int f(c - V(q)|q) h(q) dq \right] 
\end{align*}
where $f(\epsilon | q)$ is the conditional density of $\epsilon$ given $q$, and we have used the property:
\begin{align*}
\frac{\partial}{\partial c} S(c - V(q) | q) = -f(c - V(q) | q)
\end{align*}
\end{enumerate}
...
When a post idea arrives it has a quality level $q$, which affects how other users respond to the post, and the user has an intrinsic motivation to post it, $\epsilon$. 


The joint probability density function of $(q,\epsilon)$ is $f(q,\epsilon)$. We assume that $q$ and $\epsilon$ are \emph{strictly affiliated}. 
...
\end{align}
where $\text{Quality}_{ijt}$ is the measure of post quality (either zaps or replies in the first 48 hours), $\text{Cost}_{ijt}$ is the cost (in sats) of the post, $\delta_{j}$ is a set of dummy variables for each territory, and $\gamma_{t}$ is a set of dummy variables for each week. The coefficient $\alpha$ identifies the elasticity between posting cost and post quality.

The regression can be thought of as a difference-in-differences regression, in which the causal effect of posting cost is identified from differential changes in post quality across territories when territory posting costs change differentially. For example, if the posting cost in the \texttt{econ} territory increases while the posting cost in \texttt{bitcoin} stays the same, and subsequently the average quality of posts increases in \texttt{econ} relative to \texttt{bitcoin}, this would suggest that the change in posting cost likely caused the change in post quality. The coefficient $\alpha$ is identified off the many such changes present in the data.
