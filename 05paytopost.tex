...
Finally, we comment on column 4 of Table \ref{tbl_sats48_cost_reg}, where we include user fixed effects. The inclusion of user fixed effects reduces the estimated elasticity between posting cost on post quality from \gn{Zaps48CostElasticity} to \gn{Zaps48CostElasticityUserFE} (still statistically significant at the 5\% level). This suggests that the increase in post quality in response to an increase in posting cost is driven by both intensive margin (within-user) and extensive margin (compositional) effects. On the intensive margin, the same users now post higher quality posts, conditional on posting. On the extensive margin, the composition of posters in the territory shifts towards higher quality, most likely due to lower quality users dropping out. Column 3 remains our preferred specification over column 4, because we view the effect on the composition of users as a dimension we want to capture when studying the relationship between posting cost and post quality.
...
These results, combined with suggestive evidence in Figure \ref{fig_post_quality_event_study} that post quality is slightly decreasing on average prior to a fee increase, suggest that territory operators may be raising fees as a response to an influx of low quality posts, or lowers fees in response to a decline in post quantity. These actions would be rational and consistent with our hypothesized mechanism, and is consistent with anecdotal evidence from territory operators. One territory operator is quoted saying:
